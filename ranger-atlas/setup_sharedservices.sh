#!/usr/bin/env bash
# Setup Hortoniabank demo on HDP 2.6.5 Shared Services DPS Managed Cluster
# Set Ambari admin password, SSH in as root and run below:
# curl -sSL https://raw.githubusercontent.com/abajwa-hw/masterclass/master/ranger-atlas/setup_sharedservices.sh | sudo -E sh
# Add Zeppelin to the cluster before starting if the service isn't already installed

sudo su -

#Ambari admin password - replace with your own
export ambari_pass=${ambari_pass:-admin} 

#Choose password for Zeppelin users
export zeppelin_pass=password
...
#curl -u admin:${ambari_pass} -i -H 'X-Requested-By: ambari' -X PUT -d '{"RequestInfo": {"context" :"Stop FLUME via REST"}, "Body": {"ServiceInfo": {"state": "INSTALLED"}}}' http://${host}:8080/api/v1/clusters/${cluster_name}/services/FLUME
...
#sh ./01-atlas-import-classification.sh
sh ./typedefs_create.sh data/classifications.json
#./02-atlas-import-entities.sh      ## this gives 500 error so moving to end
#sh ./03-update-servicedefs.sh
sh ./servicedef_v2_updateByName.sh hive data/ranger-servicedef-hive.json+

# this doesn't work yet
#sh ./servicedef_v2_updateByName.sh tag  data/ranger-servicedef-tag.json+
...
bash -x ./05-create-hdfs-user-folders.sh
bash -x ./06-copy-data-to-hdfs.sh
...
if [ "${enable_kerberos}" = true  ]; then
   bash -x ./07-create-hive-schema-kerberos.sh
else
   bash -x ./07-create-hive-schema.sh        
fi     
